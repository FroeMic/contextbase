...
type DrawerContentProps = Omit<React.ComponentProps<typeof DrawerPrimitive.Content>, "children"> & {
  ...
  return (
    <DrawerPortal data-slot="drawer-portal">
      {backgroundClassName ? (
        <div
          aria-hidden="true"
          className={cn("pointer-events-none fixed inset-0 z-40", backgroundClassName)}
          data-slot="drawer-background"
        />
      ) : null}
...
function DrawerHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex max-w-full min-w-0 flex-col gap-0.5 p-4 group-data-[vaul-drawer-direction=bottom]/drawer-content:text-center group-data-[vaul-drawer-direction=top]/drawer-content:text-center md:gap-1.5 md:text-left",
        className,
      )}
      {...props}
    />
...
function DrawerFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("mt-auto flex max-w-full min-w-0 flex-col gap-2 p-4", className)}
      {...props}
    />
  )
}
...
function DrawerTitle({ className, ...props }: React.ComponentProps<typeof DrawerPrimitive.Title>) {
  return (
    <DrawerPrimitive.Title
      ...
      className={cn(
        "min-w-0 max-w-full font-heading text-base font-medium text-foreground",
        className,
      )}
      {...props}
    />
...
function DrawerDescription({
  ...
  return (
    <DrawerPrimitive.Description
      ...
      className={cn("min-w-0 max-w-full text-sm text-muted-foreground", className)}
      {...props}
    />
  )
}
