/* biome-ignore-all lint: generated shadcn sidebar-08 primitive persists collapse state in a cookie and keeps registry hook dependencies. */
...
function getSidebarCookieOpenState(defaultOpen: boolean) {
...
function syncSidebarRootOpenState(open: boolean) {
  if (typeof document === "undefined") return

  document.documentElement.setAttribute("data-sidebar-state", open ? "expanded" : "collapsed")
}

function isSidebarShortcutEditableTarget(target: EventTarget | null) {
...
function SidebarProvider({
  ...
  const [_open, _setOpen] = React.useState(() => getSidebarCookieOpenState(defaultOpen))
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      syncSidebarRootOpenState(openState)
    },
    [setOpenProp, open],
  )

  React.useEffect(() => {
    syncSidebarRootOpenState(open)
  }, [open])
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      if (event.key === SIDEBAR_KEYBOARD_SHORTCUT && (event.metaKey || event.ctrlKey)) {
        if (isSidebarShortcutEditableTarget(event.target)) return
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-lg px-2 text-left text-sm ring-sidebar-ring outline-hidden transition-[width,height,padding] group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-7! group-data-[collapsible=icon]:p-1.5! hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground [&_svg]:size-4 [&_svg]:shrink-0 [&>span:last-child]:truncate",
  {
    variants: {
      variant: {
  ...
  return (
    <ul
      ...
      className={cn(
        ...
        variant === "default" && "mx-3.5 translate-x-px border-l border-sidebar-border px-2.5",
        className,
      )}
      {...props}
    />
    ...
    props: mergeProps<"a">(
      {
        className: cn(
          ...
          variant === "default" && "-translate-x-px",
          className,
        ),
      },
      props,
